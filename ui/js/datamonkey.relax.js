...
function default_tree_settings () {
    ...
    tree.branch_length (function (n) {if (branch_lengths) {return branch_lengths [n.name] || 0;} return undefined;});
...
render_color_scheme = function (svg_container, attr_name, do_not_render) {
    ...
    if (branch_annotations && ! do_not_render) {
       ...
       g_container.append ("rect").attr ("x", 0)
        ...
        x_label.text (function (d) {return $('<textarea />').html(d).text();})
...
render = function (json) {
    ...
    try {
        ...
        partition_list = partition_list.selectAll ("a").data (function (d) {return d;});
        partition_list.enter ().append ("a");
        partition_list.attr ("href", "#").on ("click", function (d,i) {d3.select ("#datamonkey-relax-tree-highlight").attr ("value", d); render_tree ();});
        partition_list.text (function (d) {if (d == "RELAX.test") {this.click();} return d;});
...
function edge_colorizer (element, data) {
    ...
    if (branch_annotations) {
        ...
        $(element[0][0]).tooltip({'title' : omega_format(branch_annotations[data.target.name]), 'html' : true, 'trigger' : 'hover', 'container' : 'body', 'placement' : 'auto'})
    } else {
	...
	omegaPlot = function (data_to_plot, secondary_data, settings) {
        ...
        if (secondary_data) {
            ...
            reference_omega_lines.transition().attr ("x1", function (d) {return omega_scale(d.omega);})
                                ...
                                .style ("stroke", function (d) {return "#1f77b4";})
		...
		omega_lines.transition().attr ("x1", function (d) {return omega_scale(d.omega);})
                            ...
                            .attr ("y1", function (d) {return proportion_scale(-0.05);})
                            .attr ("y2", function (d) {return proportion_scale(d.weight);})
                            .style ("stroke", function (d) {return "#d62728";})
