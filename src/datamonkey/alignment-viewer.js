...
function datamonkey_alignment_viewer(options) {
  ...
  function draw() {
    ...
    //initial draw
    self.paper = d3
      .selectAll(self.container.selector)
      .append("svg")
      .attr("width", width)
      .attr("height", height);
    ...
    self.yAxis = self.paper.append("g").attr(yAxisAttrs);

    self.yAxisLabels = self.yAxis.append("g").attr("class", "sequence-labels");

    var legenedHLabel = self.yAxisLabels
      .selectAll("sequence-label")
      .data(self.dataset);

    var legenedHLabelAttrs = {
      class: "sequence-label",
      y: function(d, i) {
        return i * self.cell_size + offset;
      },
      fill: "#333",
    ...
    legenedHLabel.exit().remove();

    self.xScale = d3.scale
      .linear()
      .domain([
        (function() {
          if (d3.min(values) < 0) {
    ...
    self.xAxisHeader = self.paper.append("g").attr(xAxisHeaderAttrs);

    self.xAxisHeader.style("fill", "none").call(self.xAxis);

    self.yScale = d3.scale
      .ordinal()
      .domain(d3.range(self.dataset.length))
      .rangeRoundBands([0, height], 0.25);

    var xGridAttrs = {
      transform: "translate(0," + self.yScale(self.dataset.length) + ")",
      fill: "#ccc",
      "font-size": "12px",
      "font-family":
        '"Helvetica Neue", Helvetica, Arial, "Lucida Grande", sans-serif'
    ...
    self.xAxisHeader
      .selectAll("g.xAxis.header g text")
      .style(xAxisHeaderStyles);
    var xAxisTickLineStyles = { stroke: "#CCC" };
    self.xAxisHeader
      .selectAll("g.guide_lines_x g line")
      .style(xAxisTickLineStyles);
  }
