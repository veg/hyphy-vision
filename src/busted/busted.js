...
function busted_render_summary(json) {
  ...
  var fitted_distributions =
    json["fits"]["Unconstrained model"]["rate distributions"];
  ...
  for (var b in fitted_distributions) {
    ...
    try {
      ...
      for_branch_table[branch_count][4] =
        json["test results"][b]["uncorrected p"];
    } catch (e) {}
  ...
  d3
    .select("#summary-test-result")
    .text(json["test results"]["p"] <= 0.05 ? "evidence" : "no evidence");
  d3
    .select("#summary-test-pvalue")
    .text(d3.format(".3f")(json["test results"]["p"]));
  d3
    .select("#summary-pmid")
    .text("PubMed ID " + json["PMID"])
    .attr("href", "http://www.ncbi.nlm.nih.gov/pubmed/" + json["PMID"]);
  d3
    .select("#summary-total-runtime")
    .text(format_run_time(json["timers"]["overall"]));
  ...
  for (k = 0; k < 2 + has_background; k++) {
    ...
    try {
      model_rows[k].push(
        only_distro
          ? ""
          : fit_format(json["fits"][access_key]["log-likelihood"])
      );
      model_rows[k].push(
        only_distro ? "" : json["fits"][access_key]["parameters"]
      );
      model_rows[k].push(
        only_distro ? "" : fit_format(json["fits"][access_key]["AIC-c"])
      );
      model_rows[k].push(
        only_distro ? "" : format_run_time(json["fits"][access_key]["runtime"])
      );
      model_rows[k].push(
        only_distro ? "" : fit_format(json["fits"][access_key]["tree length"])
      );

      for (j = 0; j < 3; j++) {
        model_rows[k].push(
          omega_format(
            json["fits"][access_key]["rate distributions"][secondary_key][j][0]
          ) +
            " (" +
            prop_format(
              json["fits"][access_key]["rate distributions"][secondary_key][
                j
              ][1]
            ) +
            ")"
...
function busted_render_histogram(c, json) {
  ...
  erc.forEach(function(elem, i) {
    model_results.push({
      ...
      er_optimized_null: Math.log(
        json["evidence ratios"]["optimized null"][0][i]
      )
    });
  });
  ...
  composite
    ...
    .legend(
      dc.legend().x($(window).width() * 0.8 - 150).y(20).itemHeight(13).gap(5)
    )
  ...
  dc
    ...
    .group(function(d) {
      return (
        site_index.bottom(1)[0].site_index +
        " - " +
        site_index.top(1)[0].site_index
      );
    })
  ...
  $("#apply-thresholds").on("click", function(e) {
    var erConstrainedThreshold = document.getElementById(
      "er-constrained-threshold"
    ).value;
    var erOptimizedNullThreshold = document.getElementById(
      "er-optimized-null-threshold"
    ).value;
