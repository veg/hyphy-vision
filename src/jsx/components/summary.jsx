...
var Summary = React.createClass({
  ...
  componentDidMount: function() {
    this.setState({
      ...
      direction: this.props.json["fits"]["Alternative"]["K"] > 1
        ? "intensification"
        : "relaxation",
      evidence: this.state.p <= this.props.alpha_level
        ? "significant"
        : "not significant",
      ...
      <div className="col-md-12">
        <ul className="list-group">
          <li className="list-group-item list-group-item-info">
            ...
            >
              Test for selection{" "}
              <strong id="summary-direction">{this.state.direction}</strong>
              (<abbr title="Relaxation coefficient">K</abbr> ={" "}
              <strong id="summary-K">{this.state.summary_k}</strong>) was{" "}
              <strong id="summary-evidence">{this.state.evidence}</strong>
              (p = <strong id="summary-pvalue">{this.state.p}</strong>,{" "}
