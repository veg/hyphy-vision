...
class BaseGraph extends React.Component {
  ...
  computeRanges() {
      ...
      return {
          ...
          y_range : d3.extent (_.flatten (_.map (self.props.y, function (data_point) {return d3.extent (data_point);})))
      };
  }
  ...
  renderAxis (scale, location, label, dom_element) {
      ...
      var xAxis = d3.svg.axis()
       ...
       self.doTransition(d3.select(dom_element))
            .call(xAxis);
  ...
  render() {
    ...
    var XDimensionOptions = _.map(['Site'].concat(self.props.headers), function (value) {
                              return self.dimensionOptionElement(self.xAxis, value);
                            }, self);

    var YDimensionOptions = _.map(['Site'].concat(self.props.headers), function (value) {
      ...
      return (
          <div>
            ...
            <svg width = {this.props.width} height = {this.props.height}>
                <g transform = {"translate("+ this.props.marginLeft + "," + this.props.marginTop + ")"} ref = {_.partial(this.renderGraph, x_scale, y_scale).bind(this)}/>
                {
                  this.props.xAxis ?
                  (<g {...this.makeClasses ("axis")} transform = {"translate(" + this.props.marginLeft + "," + (main.height+this.props.marginTop+this.props.marginXaxis) + ")"} ref = {_.partial (this.renderAxis, x_scale, "bottom", this.props.xLabel).bind(this)}/>):
                  null
                }
                {
                  this.props.yAxis ?
                  (<g {...this.makeClasses ("axis")} transform = {"translate(" + (this.props.marginLeft-this.props.marginYaxis) + "," + this.props.marginTop + ")"} ref = {_.partial (this.renderAxis, y_scale, "left", this.props.yLabel).bind(this)}/>):
                  null
                }
            </svg>
          </div>
...
class ScatterPlot extends BaseGraph {
  ...
  renderGraph(x_scale, y_scale, dom_element) {
     ...
     _.each (this.props.y, _.bind (function (y, i) {
          ...
          this.doTransition(data_points.classed ("series_" + i, true)).attr ("cx", function (d) {return x_scale (d[0]);})
...
class Series extends BaseGraph {
  ...
  renderGraph (x_scale, y_scale, dom_element) {
   ...
   _.each(self.props.y, function (y, i) {
        ...
        self.doTransition(data_points.classed ("series_" + i, true)).attr("d", series_line)
        ...
        if (self.props.doDots) {
          ...
          self.doTransition(data_points.classed ("series_" + i, true)).attr ("cx", function (d) {return x_scale (d[0]);})
                     .attr ("cy", function (d) {return y_scale(d[1]);})
