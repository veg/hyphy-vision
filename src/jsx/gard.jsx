...
function GARDResults(props) {
  ...
  var percentageExplored = (100 *
    totalModelCount /
    totalPossibleModels).toFixed(2);
  var evidence_statement = props.data.improvements
    ? <span>
        <strong className="hyphy-highlight">found evidence</strong> of{" "}
        {props.data.lastImprovedBPC} recombination breakpoint{props.data.lastImprovedBPC == 1 ? "" : "s"}
      </span>
    : <span>
        ...
        <td>
          {(props.data.baselineScore -
            _.first(
              ...
              index
            ).reduce((t, n) => t + n, 0)).toFixed(2)}
        </td>
        <td>
          {row.deltaAICc ? row.deltaAICc.toFixed(2) : ""}
        </td>
        <td>
          {segments[index]}
        </td>
...
function GARDTopologyReport(props) {
  ...
  for (var pcounter = 1; pcounter < readPCount; pcounter++) {
    ...
    rows.push(
      <tr>
        <td>
          {_.last(props.data.improvements).breakpoints[pcounter - 1] + 1}
        </td>
        <td>
          {Math.min(1, lhs * totalComparisons)}
        </td>
        <td>
          {Math.min(1, rhs * totalComparisons)}
        </td>
        <td>
          {sigCell}
        </td>
    ...
    <div className="row">
      <div className="col-md-12">
        ...
        <p className="description">
          ...
          that allows for different topologies between segments ({currentAIC.toFixed(1)}),
        ...
        <div className="container">
          <div className="row">
            ...
            <div className="col-md-12 col-lg-10">
              ...
              <RateMatrix
                rate_matrix={
                  this.state.data ? this.state.data.rateMatrix : undefined
                }
              />
