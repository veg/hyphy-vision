...
class FUBARViz extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    var grid = nextProps.data.map(row => [
        +row[0].toFixed(2),
        +row[1].toFixed(2),
        +row[2]
      ]),
    ...
    this.x = d3.scale.ordinal().domain(gridpoints).rangePoints([0, width], 1);

    this.y = d3.scale.ordinal().domain(gridpoints).rangePoints([height, 0], 1);
    ...
    main.append("g").attr("class", "axis axis").call(yAxis);
    ...
    var colorbar_scale = d3.scale
      .linear()
      .domain([0, 1, 10])
      .range([height, height / 2, 0]);

    var colorbar_axis = d3.svg
      .axis()
  ...
  componentWillUpdate(nextProps, nextState) {
    if (nextState.site != this.state.site) {
      ...
      for (
        partition = 0;
        partition < _.values(nextProps.partitions).length;
        partition++
      ) {
        index = nextProps.partitions[partition].coverage[0].indexOf(
          nextState.site - 1
        );
        if (index > -1) {
          break;
      ...
      var data = nextProps.data.map((d, i) => {
        return nextState.site
          ? [d[0], d[1], nextProps.posterior[partition][index][0][i]]
          : d;
      });
  ...
  render() {
    var self = this;
    return (
      <div className="row" id="plot-tab">
        <div className="col-md-12">
          <Header
            title="Posterior rate distribution"
            popover="Clear the input box to view the alignment wide distribution."
          />
        ...
        <div className="col-md-6">
          <div
            className={
              "input-group" + (self.state.input_error ? " has-error" : "")
            }
            style={{ width: 300 }}
          >
        ...
        <div className="col-md-12">
          ...
          <p className="description">
            This graph shows the posterior distribution over the discretized
            rate grid. The size of a dot is proportional to the posterior weight
            allocated to that gridpoint, and the color shows the intensity of
            selection. Site-specific distributions can be viewed by entering a
            site number in the input box above the figure. When this is empty,
...
function FUBARTable(props) {
  ...
  var formatter = d3.format(".3f"),
    new_rows = flattened.map((row, index) => {
      var selection =
        row[4] > props.posteriorProbability ? "positive-selection-row" : "";
      selection =
        row[3] > props.posteriorProbability
    ...
    props.data.MLE.headers.map(pair => {
      return {
        value:
          pair[0] == "alpha;"
            ? "&alpha; "
            : pair[0].replace("alpha", "&alpha;").replace("beta", "&beta;"),
        abbr: pair[1],
        sortable: true
      };
    })
...
class FUBAR extends React.Component {
  constructor(props) {
    ...
    this.updatePosteriorProbability = this.updatePosteriorProbability.bind(
      this
    );
          ...
          <div className="row">
            ...
            <div className="col-md-12 col-lg-10">
              ...
              <FUBARViz
                ...
                partitions={
                  self.state.data ? self.state.data["data partitions"] : null
                }
                posterior={self.state.data ? self.state.data.posterior : null}
                number_of_sites={
                  self.state.data
                    ? self.state.data.input["number of sites"]
                    : null
                }
              />
